...
impl ConfigManager {
    ...
    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path.as_ref()
            .map(|p| AbsolutePathBuf::from_absolute_path(p.clone()))
            .transpose()?
            .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no user config path"))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path.as_ref()
                .map(|p| AbsolutePathBuf::from_absolute_path(p.clone()))
                .transpose()?
                .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no user config path"))?;
