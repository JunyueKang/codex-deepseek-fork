...
#[test]
fn test_metadata_defaults_to_remote() {
    let provider_toml = r#"
name = "Test Provider"
...
wire_api = "responses"
    ...
    let provider = toml::from_str::<ModelProviderInfo>(provider_toml).unwrap();
    assert_eq!(provider.metadata, ModelMetadataSource::Remote);
    assert!(provider.models.is_empty());
}

#[test]
fn test_metadata_local_with_models() {
    let provider_toml = r#"
name = "Local Provider"
base_url = "https://example.com/v1"
env_key = "KEY"
...
supports_reasoning_summaries = true
    ...
    let provider = toml::from_str::<ModelProviderInfo>(provider_toml).unwrap();
    assert_eq!(provider.metadata, ModelMetadataSource::Local);
    assert_eq!(provider.models.len(), 2);

    let m0 = &provider.models[0];
    assert_eq!(m0.slug, "my-model");
    assert_eq!(m0.display_name.as_deref(), Some("My Model"));
    assert_eq!(m0.context_window, Some(128_000));
    assert!(m0.supports_parallel_tool_calls); // default true

    let m1 = &provider.models[1];
    assert_eq!(m1.slug, "my-model-2");
    assert_eq!(m1.display_name.as_deref(), None); // defaults to slug in conversion
    assert_eq!(m1.context_window, Some(64_000));
    assert!(!m1.supports_parallel_tool_calls);
    assert!(m1.supports_reasoning_summaries);

    // Convert to ModelInfo and verify defaults are sensible
    let info: codex_protocol::openai_models::ModelInfo = m0.clone().into();
    assert_eq!(info.slug, "my-model");
    assert_eq!(info.display_name, "My Model");
    assert_eq!(info.context_window, Some(128_000));
    assert_eq!(info.truncation_policy.limit, 128_000);
    assert!(info.supports_parallel_tool_calls);
    assert_eq!(info.shell_type, codex_protocol::openai_models::ConfigShellToolType::ShellCommand);
    assert!(!info.used_fallback_model_metadata);

    let info2: codex_protocol::openai_models::ModelInfo = m1.clone().into();
    assert_eq!(info2.display_name, "my-model-2"); // slug fallback
    assert!(!info2.supports_parallel_tool_calls);
    assert!(info2.supports_reasoning_summaries);
}

#[test]
fn test_metadata_remote_is_default_for_existing_configs() {
    // Config without metadata field should default to Remote
    let provider_toml = r#"
name = "Legacy"
