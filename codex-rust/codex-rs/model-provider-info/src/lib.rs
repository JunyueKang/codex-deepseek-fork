...
pub enum ModelMetadataSource {
    /// Fetch model metadata from the provider's `/v1/models` endpoint.
    #[default]
    Remote,
    /// Use model metadata defined locally under `[[model_providers.<id>.models]]`.
    /// When the local list is empty, falls back to built-in fallback metadata.
    Local,
}
...
impl<'de> Deserialize<'de> for ModelMetadataSource {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
...
/// Lightweight model metadata that users can embed in their provider config.
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, JsonSchema)]
pub struct LocalModelInfo {
    /// Machine-friendly identifier (e.g. `"deepseek-v4-flash"`).
    pub slug: String,
    /// Human-readable name shown in the UI. Defaults to `slug` when omitted.
    #[serde(default)]
    ...
    /// Whether the model emits reasoning summaries.
    #[serde(default)]
...
impl From<LocalModelInfo> for ModelInfo {
    fn from(local: LocalModelInfo) -> Self {
        let display_name = local.display_name.unwrap_or_else(|| local.slug.clone());
        let context_window = local.context_window;
        let truncation_policy = local.truncation_policy.unwrap_or_else(|| {
            TruncationPolicyConfig {
                mode: TruncationMode::Bytes,
                limit: context_window.unwrap_or(128_000),
            }
        });
        let supported_reasoning_levels = if local.supported_reasoning_levels.is_empty() {
            vec![
            ...
            local.supported_reasoning_levels
        };
        Self {
            slug: local.slug,
            display_name,
            ...
            model_messages: None,
            supports_reasoning_summaries: local.supports_reasoning_summaries,
            ...
            truncation_policy,
            supports_parallel_tool_calls: local.supports_parallel_tool_calls,
...
pub struct ModelProviderInfo {
    ...
    /// Where model metadata comes from. Defaults to `remote` (fetch from `/v1/models`).
    #[serde(default)]
