...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let codex_home = find_codex_home()?;
            Ok(LoaderOverrides {
                user_config_path: Some(resolve_profile_v2_config_path(&codex_home, profile_v2).to_path_buf()),
...
async fn run_interactive_tui(
    ...
    let loader_overrides_for_closure = loader_overrides.clone();
    let start_tui = move || {
        codex_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            loader_overrides_for_closure.clone(),
