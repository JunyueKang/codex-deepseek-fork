...
如果只改了 CLI 和 loader，没有把 `LoaderOverrides` 存进 `App` 并传给 `rebuild_config_for_cwd()`，会出现运行时 bug：刚启动直接对话正常，但 `/resume` 后报 `Model provider 'my-provider' not found`。
...
- `/resume` 报 `Model provider 'my-provider' not found`：检查 `App` 是否保存了 `loader_overrides`，以及 `rebuild_config_for_cwd()` 是否调用 `.loader_overrides(self.loader_overrides.clone())`。
...
2. 修正 provider 覆盖后，恢复流程读到了历史 provider `my-provider`，但运行期重建配置时没有复用启动时的 loader 配置层，导致报 `Model provider 'my-provider' not found`。
...
- DeepSeek/MyProvider 会话被错误地套到 OpenAI provider 上。
...
但没有保存 `loader_overrides`。因此启动后如果执行 `/resume`，运行期重新 build config 时会少加载一层配置，可能找不到历史会话里的 provider，例如 `my-provider`。
...
这样恢复会话时，app-server 读取到历史 provider `my-provider` 后，重新加载配置也能找到 `model_providers.my-provider`。
...
这组改动恢复并实现 `wire_api = "chat"`，用于 DeepSeek、MyProvider、Ollama 兼容端、OpenAI-compatible 聚合网关等只支持 `/v1/chat/completions` 的 provider。
...
`--api_config` 如果只在启动时生效，但 `/resume` 时重建 config 丢掉它，就会复现 `Model provider 'my-provider' not found`。因此 `/resume` 修复里的 `loader_overrides` 传递同时也是 `--api_config` 的必要补丁。
